...
7. **스냅샷만 많은 옛 사본이 매매기록을 말없이 덮지 않는다** (실제로 터졌던 경로)
8. **서버를 못 읽었을 때 이 기기 사본으로 서버를 덮지 않는다** — 조회 실패를
   "서버가 비었다"로 보면 남의 기록 위에 이 기기 것을 올려버린다
9. **부팅 때 서버를 못 읽어도, 연결되면 미뤄둔 기록이 올라간다** — 8번을 막느라
   맞춰보기를 건너뛰면, 네트워크가 돌아와도 영영 이 기기에만 쌓인다
